...
r.post('/',                   authorize('student'), upload.array('attachments', 5), validateUploadedFile, createLogbookEntry);
r.post('/sync',               authorize('student'), syncOfflineEntries);
r.put('/:id',                 authorize('student'), upload.array('attachments', 5), validateUploadedFile, updateLogbookEntry);
